...
/* BUG FOUND & FIXED (deep audit pass): 11 entries below had gone stale —
   documents-data.js titles picked up parenthetical suffixes like "(CLP)",
   "(MBL)", "(eBRC)" after this array was first written, so
   WORKFLOW_ORDER.indexOf(docTitle) was silently returning -1 for those 11
   documents (their "Follows: X" hint and workflow-chain diagram node just
   went missing — no crash, just quietly absent). Also found 12 real
   documents that had NO entry here at all (Marine Insurance Policy, RCMC,
   LUT, SOFTEX, FCR, MTD, EGM, CFS Gate Pass, Cargo Survey/Damage Claim
   Report, ISF 10+2, Transshipment Permit, GST e-Invoice) — same silent
   symptom. Verified by cross-checking every title here against
   DOCUMENT_CONFIGS' actual titles programmatically; all 102 real titles
   now have exactly one entry, in a logically clustered position. */
const WORKFLOW_ORDER = [
  ...
  "Certificate of Origin","GSP Certificate of Origin (Form A)","Certificate of Analysis","Fumigation Certificate","Phytosanitary Certificate","Weight Certificate","Halal / Kosher Certificate",
  "Insurance Certificate","Marine Insurance Policy","Letter of Credit","Bill of Exchange","Bank Guarantee",
  "Export License","Letter of Undertaking (LUT)","SOFTEX Form (Software Export Declaration)","Export Declaration","Dangerous Goods Declaration","MSDS",
  ...
  "Container Load Plan (CLP)","Container Packing Certificate","Equipment Interchange Receipt (EIR)","Seal Report",
  "Vehicle Inspection Report","Road Permit","Trip Sheet","Delivery Challan","Dispatch Note","Lorry Receipt","Weighbridge Slip","Dock Receipt","CFS Gate Pass",
  "ARE-1","Shipping Bill","EDI Copy","Export General Manifest (EGM)","Inspection Certificate","Mate Receipt",
  "Master Bill of Lading (MBL)","House Bill of Lading (HBL)","Bill of Lading","Multimodal Transport Document (MTD)","Freight Invoice (Sea)","Sea Waybill","Air Waybill","ULD Manifest","Air Freight Invoice",
  "Cargo Manifest","Arrival Notice","Importer Security Filing — ISF (10+2)","Transshipment Permit","Container Destuffing Report",
...
/* BUG FIX (root cause of a real dead-end): this used to check EVERY direct
   prerequisite against full completion history, with no idea a Job even
   existed. So a 2-document Job like "CFS Gate Pass + Customs Gate Pass"
   would permanently block Customs Gate Pass on "Out of Charge" — a
   document that was never part of that Job and, with the auto-add
   prerequisite toggle off, was never meant to be. There was no way to
   finish that Job at all.
   Fix: when the student has an active Job with a document scope, a
   prerequisite that ISN'T in that scope no longer counts as "missing" —
   the Job's own picked document list defines what's actually being
   tested for that Job, not the full 102-document real-world chain. A
   prerequisite that IS in scope still has to be completed first, same as
   always — this only stops blocking on prerequisites the student's
   current Job never asked for in the first place. Outside of any active
   Job (or a Job with no scope restriction), behavior is unchanged. */
function skCheckPrerequisites(userId, slug){
  ...
  const activeJob = (typeof skGetActiveJob === "function") ? skGetActiveJob(userId) : null;
  const jobScope = (activeJob && typeof skGetJobDocScope === "function") ? skGetJobDocScope(activeJob) : [];
  const jobScopeSet = jobScope.length > 0 ? new Set(jobScope) : null;
  const relevantPrereqs = jobScopeSet ? prereqs.filter(p => jobScopeSet.has(p)) : prereqs;
  const missing = relevantPrereqs
